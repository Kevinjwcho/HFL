rm(list=ls())
setwd("~/Desktop/simulation/beta1/NGR") # change
library(psych)
library(fda)
library(glmnet)
...
load("~/Desktop/simulation/beta1/NGR/bt1_ss_n100_SNR2.RData") # change
...
save(alpha_gbr_AIC2, tau_gbr_AIC2, alpha_gbr_BIC2, tau_gbr_BIC2, 
     b_gbr_AIC2, b_gbr_BIC2, file="bt1_NGR_n100_SNR2.RData")     
...
par(mfrow = c(1,1))
tobs_check=seq(0,1,length.out = 101)
beta_hat_gbr  = apply(b_gbr_BIC2, 2, FUN = betahat)
plot(tobs_check, beta_true, type="l", ylim = c(-0.5, 2.5), ylab = expression(beta(t)), 
     xlab = "t", main = "BIC2")
lines(seq(0,1,length.out=101), beta_hat_gbr[,1], col="red")
matlines(tobs_check, beta_hat_gbr)
lines(tobs_check, beta_true, lwd = 3)